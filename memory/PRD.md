...
- **Iter 7 (Call signaling fixes — 2026-02)** — Fixed two P0 call/chat state bugs:
  1. **Infinite ring on provider reject**: caller's 2.5s `call_request` retry was firing fresh FCM on every retry, re-launching IncomingCallActivity. Added server-side `ringingSessions` map to dedupe FCM/webpush within a 60s window per caller/callee pair.
  2. **Ghost auto-accept when caller cancels before callee accepts**: caller's hang up sent generic `call_end` which providers couldn't observe (no listener). Added new `call_cancel` / `chat_cancel` socket events. Server fires `type=call_cancel` FCM + webpush back to the relevant device so native `IncomingCallActivity` (via `ACTION_DISMISS_CALL` broadcast) and webview heads-up notifications dismiss automatically. Server also blocks late `call_accept` / `chat_accept` if session state is `cancelled` or `rejected` — caller no longer gets billed for a ghost call.
  3. Caller `CallScreen` / `ChatScreen` now emit `call_cancel` / `chat_cancel` while still ringing (phase !== connected) instead of `call_end`. Provider `ProviderHome` listens for these events and dismisses the in-app incoming dialog. Disconnect handler also broadcasts cancel/reject for in-flight ringing sessions.
  4. Backend integration tests at `/app/node-backend/tests/test_signaling.js` and `/app/node-backend/tests/test_dedup.js` verify relay + ghost-accept protection.
